...
void MBL_LSL_R::setup(int _preferredNumHeadsets, float _R_InitValue) {
	lslIn.start();
...
void MBL_LSL_R::updateWavelengths(vector<float> &_input, vector<float> &_output) {
...
//--------------------------------------------------------------
void MBL_LSL_R::updateMapping(std::vector<pair<string, string> > _mapping) {
...
//--------------------------------------------------------------
vector<int> MBL_LSL_R::getStability(std::string _color, vector<ofxStability> _stab) {
	for (auto& s: _stab) {
		if (s.color == _color) {
			return s.sample;
		}
...
float MBL_LSL_R::getColorCorrelation(rColor _color, rType _type) {
	return getColorCorrelation(rColorNames[_color], _type);
}

float MBL_LSL_R::getColorCorrelation(string _color, rType _type) {
...
float MBL_LSL_R::getPairCorrelation(string _colorA, string _colorB, rType _type){
	pair<string, string> cPair = make_pair (_colorA, _colorB);
	return getPairCorrelation(cPair, _type);
}

float MBL_LSL_R::getPairCorrelation(pair<string, string> _color, rType _type) {
	pair<int, int> cPair = make_pair (getIndexFromColor(_color.first), getIndexFromColor(_color.second));
	return getPairCorrelation(cPair, _type);
	
}

float MBL_LSL_R::getPairCorrelation(pair<int,int> _indices, rType _type) {
	int mapIndex = getMapIndexFromPair(_indices);
	return (pairR[mapIndex][_type]);
}

pair<string, string> MBL_LSL_R::getHighCorrelationPair(rType _type) {
	return getColorPairFromMapIndex(highPairMapIndex[_type]);
}

pair<string, string> MBL_LSL_R::getLowCorrelationPair(rType _type) {
	return getColorPairFromMapIndex(lowPairMapIndex[_type]);
}

//--------------------------------------------------------------
int MBL_LSL_R::getMapIndexFromPair(pair<int, int> _pair) {
	int low = min(_pair.first, _pair.second);
	int high = max(_pair.first, _pair.second);
	
	if (high >= numHeadsets) {
		ofLogWarning("MBL_LSL_R::getMapIndexFromPair") << high << " out of range of " << numHeadsets;
		return R_InitValue;
	}
...
//--------------------------------------------------------------
pair<int, int> MBL_LSL_R::getPairFromMapIndex(int _index) {
	
	pair<int, int> cPair = make_pair(0,0);
	if (_index >= numMaps) {
		ofLogWarning("MBL_LSL_R::getPairIndexFromMapIndex") << _index << " out of range of " << numMaps;
		return cPair;
	}
...
pair<string, string> MBL_LSL_R::getColorPairFromMapIndex(int _index) {
...
//--------------------------------------------------------------
int MBL_LSL_R::getNumberOfHeadSetsFromMapping(int _size){
...
//--------------------------------------------------------------
string MBL_LSL_R::getNameFromIndex(int _value) {
	if (_value >= numHeadsets) {
		ofLogVerbose("MBL_LSL_R::getNameFromIndex") << "index " << _value << " out of range, 0.." << numHeadsets;
		return "no_name";
	}
	return headSetNames[_value];
}

string MBL_LSL_R::getColorFromIndex(int _value) {
	if (_value >= numHeadsets) {
		ofLogVerbose("MBL_LSL_R::getColorFromIndex") << "index " << _value << " out of range, 0.." << numHeadsets;
		return "no_color";
	}
	return headSetColors[_value];
}

int MBL_LSL_R::getIndexFromName(string _value) {
	for (int i=0; i<headSetNames.size(); i++) {
		if (_value == headSetNames[i]) {
			return i;
		}
	}
	ofLogVerbose("MBL_LSL_R::getIndexFromName") << "name " << _value << " not found";
	return 0;
}

int MBL_LSL_R::getIndexFromColor(string _value) {
	for (int i=0; i<headSetColors.size(); i++) {
		if (_value == headSetColors[i]) {
			return i;
		}
	}
	ofLogVerbose("MBL_LSL_R::getIndexFromColor") << "color " << _value << " not found";
	return 0;
}

bool MBL_LSL_R::getColorActive(string _value) {
	for (int i=0; i<headSetColors.size(); i++) {
		if (_value == headSetColors[i]) {
			return true;
		}
